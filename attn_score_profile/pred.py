...
#  ):
#      model.config.output_attentions = True  # Attention 추적을 위한 설정
#
#      pbar = tqdm(data)
...
#              do_sample=False,
#              temperature=1.0,
#              output_attentions=True,  # Attention 스코어 반환
#          )
#
#          # Attention 추적
...
#                  avg_attention = layer_attention.mean(dim=1).cpu().numpy()  # 평균 attention score
#                  plt.figure(figsize=(10, 6))
#                  plt.hist(avg_attention.flatten(), bins=100, range=(0, 1), alpha=0.75, color="blue")  # 0~1 범위에 100개 bins
#                  plt.title(f"Attention Scores Histogram - Layer {layer_idx + 1}, Sample {idx}")
#                  plt.xlabel("Attention Score")
#                  plt.ylabel("Frequency")
#                  plt.grid(True)
#                  plt.savefig(f"attention_histograms/{model_name}_{dataset}_layer_{layer_idx + 1}_sample_{idx}.png")
#                  plt.close()
#          break
...
def load_model_and_tokenizer(path, model_name):
    ...
    if args.method == "duo_attn":
        ...
        ## SMS' COMMENTS ## . 2024-10-31
        # Below code changes the code to old version that uses flash attention
        # Can't use output attention option #
